...
def receive_sms(ser):
    ...
    while True:
        with serial_lock:
            if waitingForReply:  # Actively process messages only if we are expecting a reply
                retry_count = 0
                ...
                time.sleep(10)  # Check less frequently if not expecting a reply
        time.sleep(1)
